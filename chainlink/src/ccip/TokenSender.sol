...
contract TokenSender is Ownable {
    ...
    error TokenSender__InsufficientBalance(IERC20 token, uint256 currentBalance, uint256 requiredAmount);
    error TokenSender__NothingToWithdraw();
        ...
        returns (bytes32 messageId)
    {
        if (_amount > USDC_TOKEN.balanceOf(msg.sender)) {
            revert TokenSender__InsufficientBalance(USDC_TOKEN, USDC_TOKEN.balanceOf(msg.sender), _amount);
        }
        ...
        if (ccipFee > LINK_TOKEN.balanceOf(address(this))) {
            revert TokenSender__InsufficientBalance(LINK_TOKEN, LINK_TOKEN.balanceOf(address(this)), ccipFee);
        }
    ...
    function withdrawToken(
        ...
        if (amount == 0) revert TokenSender__NothingToWithdraw();
